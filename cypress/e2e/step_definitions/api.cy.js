...
describe("Search Register", () => {
  ...
  it("List Of All Registers", () => {
    cy.request({
      ...
      expect(response.body[0].company.catchPhrase).to.equal(
        "Multi-layered client-server neural-net"
      );
      expect(response.body[0].company.bs).to.equal(
      ...
      expect(response.body[1].company.catchPhrase).to.equal(
        "Proactive didactic contingency"
      );
      expect(response.body[1].company.bs).to.equal(
